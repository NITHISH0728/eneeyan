    ...
    <div className="flex flex-col gap-6 font-sans">
        <div className="flex justify-between items-center mb-2">
            <div>
                ...
                <p className="text-[#64748b]">Create and manage coding challenges.</p>
            </div>
            <button onClick={() => setShowModal(true)} className="bg-[#005EB8] hover:bg-[#004a94] text-white px-6 py-3 rounded-xl font-bold flex items-center gap-2 transition-all shadow-lg shadow-blue-200">
        ...
        <div className="grid grid-cols-1 gap-4">
            {tests.length === 0 ? (
                ...
                tests.map((test) => (
                    <div key={test.id} className="bg-[#F8FAFC] p-6 rounded-2xl border border-[#cbd5e1] flex justify-between items-center hover:shadow-lg hover:-translate-y-1 transition-all duration-300">
                        <div className="flex items-center gap-5">
                            <div className="w-14 h-14 bg-blue-50 rounded-2xl flex items-center justify-center text-[#005EB8] border border-blue-100">
                                <Code size={28} />
                            </div>
                            <div>
                                <h3 className="font-bold text-[#1e293b] text-lg">{test.title}</h3>
                                <div className="flex gap-4 text-sm text-[#64748b] mt-1 font-medium">
                                    <span className="flex items-center gap-1"><span className="w-2 h-2 bg-[#87C232] rounded-full"></span> {test.time_limit} mins</span>
                                    <span className="bg-[#E2E8F0] px-2 py-0.5 rounded text-xs font-mono text-[#475569] border border-[#cbd5e1]">Key: {test.pass_key}</span>
                                </div>
                            </div>
                        </div>
                        ...
                        <button 
                            ...
                            className="px-4 py-2 bg-white border border-[#cbd5e1] rounded-lg text-[#005EB8] font-bold text-sm hover:bg-[#005EB8] hover:text-white transition-all flex items-center gap-2 shadow-sm"
                        >
                    ...
                    <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.95, opacity: 0 }} className="bg-[#F8FAFC] rounded-2xl shadow-2xl w-full max-w-5xl max-h-[95vh] overflow-y-auto flex flex-col border border-[#cbd5e1]">
                        ...
                        <div className="p-8 space-y-8">
                            ...
                            <div className="flex gap-8">
                                ...
                                <div className="flex-1 space-y-5">
                                    ...
                                    <div className="flex items-start gap-3">
                                        ...
                                        <button 
                                            ...
                                            disabled={aiLoading}
                                            className="mt-7 w-[120px] h-[110px] bg-gradient-to-br from-[#005EB8] to-[#004080] text-white rounded-xl font-bold flex flex-col items-center justify-center gap-2 hover:shadow-lg transition-all active:scale-95 border border-transparent"
                                        >
                                ...
                                <div className="w-[300px] border-l border-[#cbd5e1] pl-8">
                                ...
                                <table className="w-full text-left border-collapse">
                                    <thead>
                                        <tr className="border-b border-[#cbd5e1] text-[#475569] text-xs uppercase tracking-wider">
                                            ...
                                            <th className="p-4 font-extrabold text-center">Problems Solved</th>
                                            <th className="p-4 font-extrabold text-center">Time Taken</th>
                                        </tr>
                                    </thead>
                                    <tbody className="text-sm text-[#1e293b]">
                                        {selectedTestResults.map((result, idx) => (
                                            <tr key={idx} className="border-b border-[#e2e8f0] hover:bg-white transition-colors">
                                                ...
                                                <td className="p-4 text-center font-mono text-[#64748b]">{result.time_taken}</td>
                                            </tr>
                                        ))}
                                    </tbody>
                                </table>
